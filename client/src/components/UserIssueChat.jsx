...
const UserIssueChat = () => {
  ...
  const [shopLocation, setShopLocation] = useState(null); // <-- Add this line
  ...
  useEffect(() => {
    ...
    const fetchIssue = async () => {
      const res = await fetch(
        `https://furious-repair-backend.onrender.com/api/users/issues/${issueId}`,
        { credentials: "include" }
      );
  ...
  useEffect(() => {
    // Fetch repairer shop location for the assigned repairer
    if (issue && issue.repairer && issue.repairer._id) {
      const fetchShopLocation = async () => {
        const res = await fetch(
          `https://furious-repair-backend.onrender.com/api/repairer/shop-location/${issue.repairer._id}`
        );
        const data = await res.json();
        if (data.location) {
          setShopLocation({
  ...
  const fetchChat = async () => {
    const res = await fetch(
      `https://furious-repair-backend.onrender.com/api/users/issues/${issueId}/chat`,
      { credentials: "include" }
    );
    const data = await res.json();
    setChat(data.chat);
  ...
  const handleSend = async () => {
    if (input.trim()) {
      await fetch(
        `https://furious-repair-backend.onrender.com/api/users/issues/${issueId}/chat/message`,
        {
          method: "POST",
          credentials: "include",
      ...
      <div className="bg-white rounded-lg shadow-md p-4 border-l-4 border-orange-700 mb-4 max-w-2xl mx-auto">
        ...
        {issue.description && (
          <div className="mt-2">
            ...
            {shopLocation && (
              <button
                className="mt-4 px-5 py-2 bg-green-700 text-white rounded-lg font-semibold shadow hover:bg-green-800 transition border-2 border-green-900"
                onClick={() =>
                  window.open(
                    `https://www.google.com/maps?q=${shopLocation.lat},${shopLocation.lng}`,
                    "_blank"
                  )
                }
                style={{
